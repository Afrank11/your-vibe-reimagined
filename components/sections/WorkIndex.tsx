...
export function WorkIndex({ locale }: { locale: Locale }) {
  ...
  const { archiveProjects, groups, groupLabel } = getContent(locale);
  ...
  return (
    <div>
      {/* Filters */}
      <div className="flex flex-wrap gap-2" role="group" aria-label={t.workPage.filterAria}>
        {[{ key: "All" as const, label: t.workPage.all }, ...groups].map(({ key, label }) => {
          const group = key as Filter;
      ...
      <div className="mt-12" aria-live="polite">
        <div className="rule hidden gap-6 py-3 md:grid md:grid-cols-12">
          <span className="label-mono md:col-span-1">{t.workPage.colNumber}</span>
          <span className="label-mono md:col-span-3">{t.workPage.colProject}</span>
          <span className="label-mono md:col-span-5">{t.workPage.colSummary}</span>
          <span className="label-mono md:col-span-3 md:text-right">{t.workPage.colField}</span>
        </div>
