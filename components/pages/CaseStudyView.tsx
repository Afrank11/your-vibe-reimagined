...
export function CaseStudyView({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  const { caseStudies, groupLabel } = getContent(locale);
  ...
  return (
    <article className="mx-auto max-w-site px-7 sm:px-10 pb-28 pt-32 md:px-16 xl:px-24 md:pb-40 md:pt-44">
      ...
      <div className="mb-10 flex flex-wrap items-baseline gap-x-4 gap-y-2 md:mb-14">
        <Link href={localizePath(locale, "/work")} className="link-quiet label-mono !text-brass">
          {t.caseStudy.backToWork}
        </Link>
        <span className="label-mono">{t.caseStudy.caseLabel(study.index)}</span>
      ...
      <Reveal className="mt-16 md:mt-20">
        <div className="relative aspect-[16/8] overflow-hidden bg-ink-2 md:aspect-[16/7]">
          ...
          <span className="label-mono absolute bottom-4 left-4">{groupLabel(study.group)}</span>
        </div>
      </Reveal>
          ...
          <div className="md:sticky md:top-28">
            <Reveal stagger>
              <div className="rule py-4">
                <p className="label-mono mb-2">{t.caseStudy.year}</p>
                <p className="font-mono text-xs text-bone">{study.year}</p>
              </div>
              <div className="rule py-4">
                <p className="label-mono mb-2">{t.caseStudy.field}</p>
                <p className="font-mono text-xs text-bone">{groupLabel(study.group)}</p>
              </div>
              <div className="rule py-4">
                <p className="label-mono mb-2">{t.caseStudy.role}</p>
                <p className="text-sm leading-relaxed text-silver">{study.role}</p>
              </div>
              <div className="rule py-4">
                <p className="label-mono mb-2">{t.caseStudy.stack}</p>
      ...
      <Reveal className="mt-24 md:mt-36">
        <p className="label-mono mb-4">{t.caseStudy.nextCase}</p>
        <Link
          href={localizePath(locale, `/work/${next.slug}`)}
